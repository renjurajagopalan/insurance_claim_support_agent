...
class Settings(BaseSettings):
    ...
    app_name: str = "AI copilot for Insurance Support Agents "
    groq_api_key:str = ""
    ...
    dashboard_api_url: str = "https://localhost:8000"

    def resolve(self, path: Path) -> Path:
        " Resolve the relative path against the project root"
    ...
    def chroma_rag_path(self) -> Path:
        return self.resolve(self.chroma_rag_path)
    ...
    def effective_google_embedding_model(self) -> str:
        """
        Auto upgrade legacy embeddings model to supported Gemini model
        """
...
def ensure_directories(settings: Settings | None = None) -> None:
    """
    Ceate the local directories required by SQLite and ChromaDB
    """
